...
class ChTimer {
  public:
    ...
    CH_NODISCARD inline unsigned long long GetTimeMilliseconds() const {
        if (m_running) {
    ...
    CH_NODISCARD inline unsigned long long GetTimeMicroseconds() const {
        if (m_running) {
            const auto now = std::chrono::high_resolution_clock::now();
    ...
    /// If the timer was started but not stopped, this function returns the intermediate value and the timer keeps
    /// running. Otherwise, it returns the timer value when it was stopped.
    CH_NODISCARD inline double GetTimeSeconds() const {
        if (m_running) {
            const auto int_time = std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - m_start);
    ...
    CH_NODISCARD inline double operator()() const { return GetTimeSeconds(); }
